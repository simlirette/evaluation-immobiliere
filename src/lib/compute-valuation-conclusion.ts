...
export interface ValuationConclusion {
  ...
  cv: number
  /** Overall reliability: 'elevee' | 'moderee' | 'faible' */
...
export function computeValuationConclusion(
  ...
  const reconciled = computeReconciledValue(usableAdjs)
  if (!reconciled) return null

  const prices = usableAdjs.map(a => a.adjusted)
  const averageValue = Math.round(prices.reduce((s, p) => s + p, 0) / prices.length)

  const stats = computeAdjustedPriceStats(usableAdjs)
  const cv = stats?.cv ?? 0
  ...
  const usableComps = comps.filter(isUsableComparable)
  const timeRate = usableComps.length >= 2 ? computeTimeAdjustmentRate(usableComps) : null
