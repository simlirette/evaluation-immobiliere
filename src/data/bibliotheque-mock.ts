/* Bibliothèque — données du design handoff (biblio-data.js), copie verbatim.
   4 collections : VENTES, MARCHES, COUTS, TAUX.
   À remplacer par des endpoints backend (ventes JLR/SIRF réelles, gabarits coûts MEFQ, TGA marché). */
...
export function soldLabel(v: VenteMock): string {
  const [y, mo, d] = v.soldAt.split('-').map(Number)
  return `${d} ${MONTHS[mo - 1]} ${y}`
}

export function ppsf(v: VenteMock): number | null {
  return v.area ? v.price / v.area : null
}

export const DISTRICTS = [...new Set(VENTES.map(v => v.district))]
export const VENTE_TYPES = [...new Set(VENTES.map(v => v.type))]

// Indicateurs de marché par quartier (sparkline = 8 trimestres)
...
// Coûts de remplacement neuf (approche coût)
...
// Taux de capitalisation observés
