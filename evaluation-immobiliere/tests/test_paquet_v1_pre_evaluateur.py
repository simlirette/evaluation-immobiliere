...
class TestPaquetV1PreEvaluateur(unittest.TestCase):
    def test_generate_package_from_nominal_runtime_case(self) -> None:
        ...
        try:
            ...
            self.assertEqual(manifest["package_origin"], "runtime_summary_demo")
            self.assertEqual(manifest["field_validation"], "NON_REVENDIQUEE")
            self.assertFalse(manifest["external_evaluator_responses_included"])
            self.assertFalse(manifest["sensitive_data_included"])
