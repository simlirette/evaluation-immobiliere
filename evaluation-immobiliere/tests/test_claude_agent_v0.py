...
class TestClaudeAgentV0(unittest.TestCase):
    ...
    def test_comps_market_runner_uses_search_tool_and_writes_ranked_artifacts(self) -> None:
        ...
        try:
            ...
            self.assertEqual(len(comparables_payload["comparables"]), 1)
            self.assertEqual(comparables_payload["comparables"][0]["comparable_id"], "C1")
            ...
            self.assertEqual(comparables_payload["selection_protocol"]["selected_comparable_ids"], ["C1"])
            self.assertEqual(comparables_payload["selection_protocol"]["source_coverage_status"], "OK")
            self.assertIn("source_quality", comparables_payload["selection_protocol"]["scoring_weights"])
            self.assertEqual(
                comparables_payload["source_coverage"]["schema_version"],
                "comps_market_source_coverage_v1",
            )
            self.assertEqual(comparables_payload["source_coverage"]["selected_source_ids"], ["SRC-1"])
            self.assertEqual(comparables_payload["source_coverage"]["missing_source_count"], 0)
            self.assertEqual(
            ...
            self.assertEqual(justifications_payload["justifications"][0]["decision"], "retenu")
            self.assertEqual(
            ...
            self.assertEqual(source_index_payload["sources"][0]["source_id"], "SRC-1")
            self.assertEqual(source_index_payload["sources"][0]["selected_comparable_ids"], ["C1"])
        finally:
            shutil.rmtree(root, ignore_errors=True)
    ...
    def test_compliance_qa_runner_validates_outputs_before_writing_artifacts(self) -> None:
        ...
        try:
            ...
            self.assertEqual(rapport["compliance_decision_matrix"]["status"], "PRET_REVISION_FINALE")
            self.assertEqual(rapport["compliance_decision_matrix"]["active_findings_count"], 0)
            self.assertTrue(rapport["compliance_decision_matrix"]["ok"])
            ...
            self.assertIn("traceability_log", rapport["evidence_map"]["declared_inputs"])
            ...
            self.assertEqual(statut["status"], "PRET_REVISION_FINALE")
            self.assertEqual(statut["release_gate"]["schema_version"], "compliance_release_gate_v1")
            self.assertTrue(statut["release_gate"]["ready_for_redaction"])
    ...
    def test_redaction_runner_formats_documents_before_writing_artifacts(self) -> None:
        ...
        try:
            ...
            self.assertIn("# brouillon_rapport.md", rapport)
            self.assertIn("## formatting", rapport)
            self.assertIn("format_document", rapport)
            self.assertIn("redaction", rapport)
            self.assertIn("redaction_report_plan_v1", rapport)
            self.assertIn("redaction_source_appendix_v1", rapport)
            self.assertIn("redaction_human_review_gate_v1", rapport)
            self.assertIn("ne_pas_publier_sans_revision_humaine", rapport)
            self.assertIn("SRC-1", rapport)
            self.assertIn("# annexe_sources.md", annexe)
            self.assertIn("format_document", annexe)
            self.assertIn("redaction_source_appendix_v1", annexe)
            self.assertIn("redaction_citation_policy_v1", annexe)
            self.assertIn("\"allow_unsourced_claims\": false", annexe)
        finally:
            shutil.rmtree(root, ignore_errors=True)
    ...
    def test_claude_style_pipeline_runs_all_agents_into_shared_artifact_dir(self) -> None:
        ...
        try:
            ...
            self.assertEqual(result["metrics"]["tool_use_count"], 25)
            self.assertTrue(all(message["schema_version"] == "claude_message_envelope_v0" for message in result["messages"]))
            self.assertTrue(all(event["schema_version"] == "claude_runtime_event_v0" for event in result["events"]))
            self.assertTrue(result["message_envelope_summary"]["ok"])
            self.assertTrue(result["event_envelope_summary"]["ok"])
            ...
            self.assertEqual(result["handoffs_by_agent"]["data-facts"], [])
            self.assertEqual(result["handoffs_by_agent"]["comps-market"][0]["from_agent"], "data-facts")
            ...
            self.assertEqual(lineage["artifacts_count"], 16)
            self.assertEqual(lineage["handoff_edges_count"], 4)
            ...
            self.assertEqual(lineage["handoff_edges"][0]["from_agent"], "data-facts")
            self.assertEqual(lineage["handoff_edges"][0]["to_agent"], "comps-market")
            self.assertIn(
                "data-facts.source_index.json",
                [artifact["artifact_key"] for artifact in lineage["handoff_edges"][0]["artifacts"]],
            )
            ...
            self.assertEqual(source_index_record["consumed_by"], ["comps-market"])
            self.assertFalse(source_index_record["terminal"])
            self.assertIn("redaction.brouillon_rapport.md", lineage["terminal_artifact_keys"])
            transcript_path = Path(result["transcript_path"])
            self.assertTrue(transcript_path.exists())
            self.assertEqual(result["transcript_summary"]["entries_count"], len(result["messages"]))
            self.assertEqual(result["transcript_summary"]["agents_count"], 5)
            self.assertTrue(result["transcript_summary"]["validation"]["ok"])
            self.assertEqual(result["transcript_summary"]["tool_use_count"], 25)
            self.assertEqual(result["transcript_summary"]["tool_result_count"], 25)
            self.assertEqual(result["transcript_summary"]["handoff_messages_count"], 4)
            ...
            self.assertEqual(result["hook_summary"]["hook_events"]["SessionStart"], 5)
            ...
            self.assertEqual(result["hook_summary"]["hook_events"]["SessionEnd"], 5)
            ...
            self.assertEqual(len([event for event in result["events"] if event["event"] == "handoff_created"]), 4)
            self.assertEqual(len([event for event in result["events"] if event["event"] == "handoff_received"]), 4)
            self.assertEqual(len([event for event in result["events"] if event["event"] == "hook_invoked"]), 60)
            self.assertEqual(len([event for event in result["events"] if event["event"] == "artifact_written"]), 16)
            ...
            self.assertEqual(comps_source_index["handoff_context"]["from_agents"], ["data-facts"])
            self.assertIn(
                "source_index.json",
            ...
            self.assertIn("redaction_handoff_context_v1", redaction_report)
            self.assertIn("compliance-qa", redaction_report)
            self.assertIn("statut_sortie.json", redaction_report)
        finally:
            shutil.rmtree(root, ignore_errors=True)
    ...
    def test_unknown_tool_in_agent_config_is_rejected(self) -> None:
        ...
        try:
            ...
            config_path.write_text(source.replace("  - append_audit_log", "  - outil_inconnu"), encoding="utf-8")
    ...
    def test_yaml_subset_parser_keeps_block_prompts_and_nested_lists(self) -> None:
        ...
        self.assertIn("budgets", parsed)
        self.assertEqual(parsed["agent_type"], "data-facts")
        prompts = parsed["prompts"]
        self.assertIsInstance(prompts, dict)
        self.assertIn("Objectif: produire une fiche bien fiable", prompts["system_prompt_static"])
        quality = parsed["quality_gates"]
        self.assertIsInstance(quality, dict)
        self.assertEqual(quality["blocking"], ["B002_source_required", "B004_unit_consistency"])
