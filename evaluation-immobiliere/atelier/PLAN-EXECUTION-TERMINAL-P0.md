...
## Objectif
Plan prêt à lancer pour exécuter les commandes lourdes P0 avec ordre, prérequis, owner, sortie attendue, signaux d'échec et fallback.

## Prérequis globaux
- Python 3.11+ et dépendances projet installées.
- Variables runtime/secrets Aston injectées hors repo.
- Accès aux sources de données externes pour P0-02.
...
| 2 | P0-01 | `python evaluation-immobiliere/outils/simuler_runtime_engine_v0.py` | Lead Runtime | Simulation E2E sans blocage | Exception pipeline/artefact manquant | Isoler étape fautive via logs |
...
| 6 | P0-02 | `python evaluation-immobiliere/outils/executer_dossiers_pilotes_reels_v0.py` | Lead Intégration | Run E2E réel + audit JSONL | Données réelles indisponibles | Replanifier fenêtre + run anonymisé |
...
- Ne pas poursuivre homologation si l'étape 5 échoue.
